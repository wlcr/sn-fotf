...
export function useSanityLiveQuery<T = unknown>(
  query: string,
...
export function LiveContent({
  ...
  const liveData = useSanityLiveQuery(query, params, initial, {
    enabled,
    projectId,
    dataset,
    token,
  ...
  return (
    <>
      {typeof children === 'function'
        ? (children as (data: unknown) => React.ReactNode)(liveData)
        : children}
    </>
  );
}
    ...
    <div className="fixed bottom-4 right-4 z-40">
      <div
        className={`px-2 py-1 rounded-full text-xs font-medium ${
          isPreview ? 'bg-green-500 text-white' : 'bg-gray-500 text-white'
        }`}
      >
